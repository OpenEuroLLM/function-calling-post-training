#!/usr/bin/env bash
#SBATCH --job-name=phase2-centerpiece-turndrop-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 1 Experiment centerpiece_turndrop: FC-only SFT on the 5-dataset
# centerpiece with `fcanalysis.semantic_filter.drop_turns_unjustified` applied
# uniformly to v1, v2.IA, and TxT360 (Dolci and GraphSyn unaffected — no AMS
# pair). 891,494 samples = 892,826 raw − 1,332 dropped (samples whose every
# real turn was flagged as unjustified across the four-category unjustified
# set: ANTI_MANUAL_SOLVE, ANTI_UNJUSTIFIED_REFUSAL, ANTI_PRESSURE_CAVE,
# OTHER_UNJUSTIFIED).
#
# Tests turn-range removal (vs whole-sample removal in centerpiece_AMS) at
# centerpiece scale. Same FC-only recipe as the other three Wave 1 variants:
# LR 5e-5, batch 256K, 1 epoch, 4 nodes. See notes/experiments.md
# §"centerpiece_turndrop".
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_centerpiece_turndrop_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_centerpiece_turndrop_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_centerpiece_turndrop_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — FC-only recipe (identical to centerpiece_{raw,AMS,random})
RUN_NAME="${RUN_NAME:-phase2_centerpiece_turndrop_fc_sft}"
...
echo "=== Phase 2 centerpiece_turndrop: 5-dataset Centerpiece (turn-range filtered) FC SFT Training (Leonardo) ==="
