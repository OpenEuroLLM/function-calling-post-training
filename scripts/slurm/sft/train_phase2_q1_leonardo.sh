#!/usr/bin/env bash
#SBATCH --job-name=phase2-q1-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment Q1: FC-only SFT with curated Dolci + TxT360-full (390,226 samples).
#
# Q1 is the "no AMS removal" anchor in the TxT360 trio (parallel to A2 in the v1 trio).
# Compare:
#   - Q1 vs A3: "no AMS filter vs AMS-filtered" — volume-confounded ~7% (25,026 samples)
#   - Q1 vs Q2: "full vs random subsample" — pure volume effect
#
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_q1_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_q1_leonardo.sh
...
echo "=== Phase 2 Q1: Curated Dolci + TxT360-full FC SFT Training (Leonardo) ==="
