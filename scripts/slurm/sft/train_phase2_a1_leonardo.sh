#!/usr/bin/env bash
#SBATCH --job-name=phase2-a1-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment A1: FC-only SFT with curated Dolci + GraphSyn (226,462 samples).
#
# Tests GraphSyn contribution to Multi-Turn. Compare A1 vs P1.
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a1_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a1_leonardo.sh
...
echo "=== Phase 2 A1: Curated Dolci + GraphSyn FC SFT Training (Leonardo) ==="
