#!/usr/bin/env bash
#SBATCH --job-name=phase2-a2-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment A2: FC-only SFT with curated Dolci + v1-dedup (403,345 samples).
#
# Tests Nemotron v1 contribution to Multi-Turn. Compare A2 vs P1.
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a2_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a2_leonardo.sh
...
echo "=== Phase 2 A2: Curated Dolci + v1-dedup FC SFT Training (Leonardo) ==="
