#!/usr/bin/env bash
#SBATCH --job-name=phase2-kprime-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment K': FC-only SFT with curated Dolci + GS + v1-dedup (426,772 samples).
#
# Serves dual purpose: (1) pipeline bridge — curated 3-way mix comparable to
# Phase 1 I (old pipeline, 637K), and (2) TxT360 backward ablation —
# compare K' vs P2-FC to measure TxT360's contribution in the full mix.
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_kprime_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_kprime_leonardo.sh
...
echo "=== Phase 2 K': Curated Dolci + GS + v1-dedup FC SFT Training (Leonardo) ==="
