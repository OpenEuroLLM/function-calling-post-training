#!/usr/bin/env bash
#SBATCH --job-name=phase2-a5-ia-ams-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A5_ia_ams: FC-only SFT with curated Dolci + v2.interactive_agent_ams
# (458,516 samples).
#
# Tests AMS-filtering on v2.interactive_agent (11.74% AMS rate, 23,382 removed
# of 278,863). AMS variant of the 3-variant v2.IA family; compare against
# A5_ia (no AMS) and A5_ia_random (volume-matched random control).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a5_ia_ams_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a5_ia_ams_leonardo.sh
...
echo "=== Phase 2 A5_ia_ams: Curated Dolci + v2.interactive_agent-AMS FC SFT Training ==="
