#!/usr/bin/env bash
#SBATCH --job-name=phase2-centerpiece-raw-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 1 Experiment centerpiece_raw: FC-only SFT on the 5-dataset
# centerpiece (no AMS filter applied). 892,826 samples = Dolci + GraphSyn +
# v1_dedup + v2_interactive_agent + txt360_full (concatenated + shuffled,
# seed=42).
#
# Tests the aggregate AMS-vs-Random comparison at centerpiece scale; pair with
# centerpiece_AMS and centerpiece_random. FC-only recipe: LR 5e-5, batch 256K,
# 1 epoch, 4 nodes. See notes/phase2_experiments_log.md §"Wave 1 Centerpiece
# Design".
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_centerpiece_raw_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_centerpiece_raw_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_centerpiece_raw_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — FC-only recipe
RUN_NAME="${RUN_NAME:-phase2_centerpiece_raw_fc_sft}"
...
echo "=== Phase 2 centerpiece_raw: 5-dataset Centerpiece FC SFT Training (Leonardo) ==="
