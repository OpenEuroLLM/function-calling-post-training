#!/usr/bin/env bash
#SBATCH --job-name=phase2-p2-itfc-sft
#SBATCH --nodes=8
...
# Phase 2 Experiment P2: Full IT+FC SFT (~2,513,470 samples).
#
# IT-only(~1,924,533) + Dolci-FC(203,035) + GS(23,427) + v1-dedup(200,310) + TxT360-AMS(162,165).
# Pre-specified best-recipe ceiling. IT+FC recipe: LR 8e-5, batch ~1M, 2 epochs, 8 nodes.
# Compare P2 vs Phase 1 K to measure curated pipeline improvement at full scale.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_p2_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_p2_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_p2_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — IT+FC recipe (OLMo 3 paper Table 47)
RUN_NAME="${RUN_NAME:-phase2_p2_itfc_sft}"
...
echo "=== Phase 2 P2: Full IT+FC SFT Training (Leonardo) ==="
