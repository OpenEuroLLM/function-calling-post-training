#!/usr/bin/env bash
#SBATCH --job-name=nemotron-fc-sft
#SBATCH --nodes=4
...
# Experiment C (Q1): FC-only SFT with Nemotron data (converted to Dolci format) on Leonardo.
#
# Hyperparameters identical to Experiment B (Dolci FC) except max_epochs
# (both use 1 epoch, but different step counts due to different dataset sizes).
# See FC_EXPERIMENTS_LOG.md for rationale and confound analysis.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_nemotron_fc_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A/D)
#
# Usage:
#   mkdir -p logs
#   sbatch train_nemotron_fc_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/nemotron_fc_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config
RUN_NAME="${RUN_NAME:-nemotron_fc_sft}"
...
echo "=== Nemotron FC SFT Training — Experiment C (Leonardo) ==="
