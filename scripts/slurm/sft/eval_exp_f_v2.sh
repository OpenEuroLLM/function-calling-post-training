...
# Experiment F (v2): Full Dolci Instruct SFT + Nemotron FC — trimmed olmo3:adapt × 3 runs.
#
# Model: Think-SFT fine-tuned on full Dolci-Instruct-SFT + Nemotron FC (~2.47M samples, 2 epochs, step 4366).
# Checkpoint: dolci_instruct_plus_nemotron_sft_hf (converted from OLMo-core).
#
...
#   mkdir -p logs
#   sbatch --cpus-per-task=32 eval_exp_f_v2.sh
...
MODEL_PATH="${WORK_DIR}/models/dolci_instruct_plus_nemotron_sft_hf"
OUTPUT_BASE="${WORK_DIR}/eval-results/exp_f_instruct_plus_nemotron_v2"
...
echo "=== Experiment F (v2): IT+FC+Nemotron — trimmed olmo3:adapt × 3 runs ==="
