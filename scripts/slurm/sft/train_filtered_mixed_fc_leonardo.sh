#!/usr/bin/env bash
#SBATCH --job-name=filtered-mixed-fc-sft
#SBATCH --nodes=4
...
# Experiment G (Q5): FC-only SFT with Dolci FC + Filtered Nemotron FC on Leonardo.
#
# Same as Experiment E (train_mixed_fc_leonardo.sh) but uses filtered Nemotron
# data with refuse-then-call and no-tools entries removed (~280K instead of ~316K).
# Hyperparameters identical to Experiments B, C, and E (LR 5e-5, batch 256K, 1 epoch).
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_filtered_mixed_fc_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A/B/C/D/E/F)
#
# Usage:
#   mkdir -p logs
#   sbatch train_filtered_mixed_fc_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/filtered_mixed_fc_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — same as Exp B/C/E
RUN_NAME="${RUN_NAME:-filtered_mixed_fc_sft}"
...
echo "=== Filtered Mixed FC SFT Training — Experiment G (Leonardo) ==="
