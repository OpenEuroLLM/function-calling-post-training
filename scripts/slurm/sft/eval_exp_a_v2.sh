...
# Experiment A (v2): Full Dolci Instruct SFT (IT+FC) — trimmed olmo3:adapt × 3 runs.
#
# Model: OLMo-3-7B-Instruct-SFT reproduction (2.15M samples, 2 epochs, step 3252).
# Checkpoint: OLMo-3-7B-Instruct-SFT-reproduce (converted from OLMo-core).
#
...
#   mkdir -p logs
#   sbatch --cpus-per-task=32 eval_exp_a_v2.sh
...
MODEL_PATH="${WORK_DIR}/models/OLMo-3-7B-Instruct-SFT-reproduce"
OUTPUT_BASE="${WORK_DIR}/eval-results/exp_a_instruct_sft_v2"
...
echo "=== Experiment A (v2): IT+FC (Dolci Instruct SFT) — trimmed olmo3:adapt × 3 runs ==="
