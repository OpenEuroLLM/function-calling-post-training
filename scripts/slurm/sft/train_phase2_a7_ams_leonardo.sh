#!/usr/bin/env bash
#SBATCH --job-name=phase2-a7-ams-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A7_ams: FC-only SFT with curated Dolci + nemotron_terminal_ams
# (426,255 samples).
#
# Tests AMS-filtering on nemotron_terminal (40.77% AMS rate among classified;
# 12,545 removed of 235,765 within-deduped). AMS variant of the 3-variant
# terminal family; compare against A7 (no AMS) and A7_random (random control).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a7_ams_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a7_ams_leonardo.sh
...
echo "=== Phase 2 A7_ams: Curated Dolci + nemotron_terminal-AMS FC SFT Training (Leonardo) ==="
