#!/usr/bin/env bash
#SBATCH --job-name=phase2-a2prime-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment A2': FC-only SFT with curated Dolci + v1-dedup-AMS (385,927 samples).
#
# Tests AMS filtering generalization to v1 (multi-dataset support for claim 4).
# Compare A2' vs A2 (AMS effect on v1, volume-confounded ~4.3%) and A2' vs
# Phase 1 G (AMS-filtered v1 vs RTC-filtered v1, cross-pipeline).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a2prime_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a2prime_leonardo.sh
...
echo "=== Phase 2 A2': Curated Dolci + v1-AMS FC SFT Training (Leonardo) ==="
