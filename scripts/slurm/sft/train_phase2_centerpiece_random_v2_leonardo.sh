#!/usr/bin/env bash
#SBATCH --job-name=phase2-centerpiece-random-v2-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 1 v2 (clean-label) centerpiece_random_v2: FC-only SFT on the
# 5-dataset centerpiece with a uniform random subsample matched to the AMS_v2
# volume (889,325 samples, seed=42). Matched-volume control for the clean-label
# AMS_v2 / turndrop_v2 comparison. Same FC-only recipe as the v1 centerpieces:
# LR 5e-5, batch 256K, 1 epoch, 4 nodes / 16 GPUs.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_centerpiece_random_v2_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_centerpiece_random_v2_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_centerpiece_random_v2_tokenized}"
...
RUN_NAME="${RUN_NAME:-phase2_centerpiece_random_v2_fc_sft}"
...
echo "=== Phase 2 centerpiece_random_v2: clean-label-matched random FC SFT Training (Leonardo) ==="
