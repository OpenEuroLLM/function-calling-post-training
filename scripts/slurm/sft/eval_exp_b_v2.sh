...
# Experiment B (v2): Dolci FC-only SFT — trimmed olmo3:adapt × 3 runs.
#
# Model: Think-SFT fine-tuned on Dolci FC only (227K samples, 1 epoch, step 1801).
# Checkpoint: dolci_fc_sft_hf (converted from OLMo-core).
#
...
#   mkdir -p logs
#   sbatch --cpus-per-task=32 eval_exp_b_v2.sh
...
MODEL_PATH="${WORK_DIR}/models/dolci_fc_sft_hf"
OUTPUT_BASE="${WORK_DIR}/eval-results/exp_b_dolci_fc_v2"
...
echo "=== Experiment B (v2): Dolci FC-only SFT — trimmed olmo3:adapt × 3 runs ==="
