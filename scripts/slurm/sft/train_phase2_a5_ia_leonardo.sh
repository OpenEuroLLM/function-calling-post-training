#!/usr/bin/env bash
#SBATCH --job-name=phase2-a5-ia-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A5_ia: FC-only SFT with curated Dolci + v2.interactive_agent
# (481,898 samples).
#
# Tests v2.interactive_agent's contribution to BFCL Multi-Turn (customer-service
# trajectories, 490 domains, 836 system prompts). No AMS filter (no_AMS variant
# of the 3-variant v2.IA family; pair with A5_ia_ams and A5_ia_random).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a5_ia_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a5_ia_leonardo.sh
...
echo "=== Phase 2 A5_ia: Curated Dolci + v2.interactive_agent FC SFT Training (Leonardo) ==="
