#!/usr/bin/env bash
#SBATCH --job-name=phase2-a5-ia-random-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A5_ia_random: FC-only SFT with curated Dolci +
# v2.interactive_agent random subsample (458,516 samples).
#
# Random-removal control for AMS filtering on v2.IA. Random subsample of
# v2_interactive_agent.jsonl to 255,481 (matching A5_ia_ams volume) at seed=42.
# A5_ia_ams vs A5_ia_random is volume-matched, step-matched, recipe-identical;
# the comparison extends Claim 4's "AMS-targeted > random" test to v2.IA.
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a5_ia_random_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a5_ia_random_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_a5_ia_random_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — FC-only recipe
RUN_NAME="${RUN_NAME:-phase2_a5_ia_random_fc_sft}"
...
echo "=== Phase 2 A5_ia_random: Curated Dolci + v2.interactive_agent-random FC SFT Training ==="
