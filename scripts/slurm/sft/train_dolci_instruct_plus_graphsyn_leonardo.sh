#!/usr/bin/env bash
#SBATCH --job-name=dolci-instruct-graphsyn-sft
#SBATCH --nodes=8
...
# Experiment J: Full IT+FC SFT with Dolci-Instruct-SFT + GraphSyn FC on Leonardo.
#
# Hyperparameters identical to Experiment A/F (LR 8e-5, batch ~1M, 2 epochs, 8 nodes).
# Tests whether augmenting the full Dolci SFT mix with GraphSyn FC improves FC
# capability without degrading general performance.
# See fc_experiments_design.md and fc_experiments_log.md for rationale.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_dolci_instruct_plus_graphsyn_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A)
#
# Usage:
#   mkdir -p logs
#   sbatch train_dolci_instruct_plus_graphsyn_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/dolci_instruct_plus_graphsyn_tokenized}"
...
RUN_NAME="${RUN_NAME:-dolci_instruct_plus_graphsyn_sft}"
...
echo "=== Dolci Instruct + GraphSyn FC SFT Training — Experiment J (Leonardo) ==="
