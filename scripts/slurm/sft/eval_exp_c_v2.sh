...
# Experiment C (v2): Nemotron FC-only SFT — trimmed olmo3:adapt × 3 runs.
#
# Model: Think-SFT fine-tuned on Nemotron FC only (316K samples, 1 epoch, step 2229).
# Checkpoint: nemotron_fc_sft_hf (converted from OLMo-core).
#
...
#   mkdir -p logs
#   sbatch --cpus-per-task=32 eval_exp_c_v2.sh
...
MODEL_PATH="${WORK_DIR}/models/nemotron_fc_sft_hf"
OUTPUT_BASE="${WORK_DIR}/eval-results/exp_c_nemotron_fc_v2"
...
echo "=== Experiment C (v2): Nemotron FC-only SFT — trimmed olmo3:adapt × 3 runs ==="
