#!/usr/bin/env bash
#SBATCH --job-name=phase2-centerpiece-random-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 1 Experiment centerpiece_random: FC-only SFT on a uniform-random
# subsample of centerpiece_raw at the same volume as centerpiece_AMS (827,000
# samples = 892,826 raw − 65,826 uniformly random removals from the centerpiece
# as a whole, seed=42).
#
# Control for AMS selectivity: pair with centerpiece_AMS (same volume, AMS-
# flagged selection criterion vs uniformly random). FC-only recipe: LR 5e-5,
# batch 256K, 1 epoch, 4 nodes. See notes/phase2_experiments_log.md §"Wave 1
# Centerpiece Design".
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_centerpiece_random_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_centerpiece_random_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_centerpiece_random_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — FC-only recipe
RUN_NAME="${RUN_NAME:-phase2_centerpiece_random_fc_sft}"
...
echo "=== Phase 2 centerpiece_random: 5-dataset Centerpiece (uniform random control) FC SFT Training (Leonardo) ==="
