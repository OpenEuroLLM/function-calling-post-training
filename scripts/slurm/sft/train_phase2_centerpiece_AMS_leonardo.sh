#!/usr/bin/env bash
#SBATCH --job-name=phase2-centerpiece-AMS-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 1 Experiment centerpiece_AMS: FC-only SFT on the 5-dataset
# centerpiece with AMS filter applied uniformly to v1, v2.IA, and TxT360
# (Dolci and GraphSyn unaffected — no AMS pair). 827,000 samples = 892,826
# raw − 65,826 AMS-removed.
#
# Tests AMS as a uniform pipeline filter at centerpiece scale; pair with
# centerpiece_raw (no AMS) and centerpiece_random (matched-volume control).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes. See
# notes/phase2_experiments_log.md §"Wave 1 Centerpiece Design".
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_centerpiece_AMS_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_centerpiece_AMS_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_centerpiece_AMS_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — FC-only recipe
RUN_NAME="${RUN_NAME:-phase2_centerpiece_AMS_fc_sft}"
...
echo "=== Phase 2 centerpiece_AMS: 5-dataset Centerpiece (AMS-filtered) FC SFT Training (Leonardo) ==="
