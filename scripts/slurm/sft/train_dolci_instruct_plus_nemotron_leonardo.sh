#!/usr/bin/env bash
#SBATCH --job-name=dolci-nemotron-sft
#SBATCH --nodes=8
...
# Experiment F (Q4): Full IT+FC SFT with Dolci-Instruct-SFT + Nemotron FC on Leonardo.
#
# Hyperparameters identical to Experiment A (LR 8e-5, batch ~1M, 2 epochs, 8 nodes).
# Tests whether augmenting the full Dolci SFT mix with Nemotron FC improves FC
# capability without degrading general performance.
# See FC_EXPERIMENTS_LOG.md and fc_experiment_plan.md for rationale.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_dolci_instruct_plus_nemotron_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A)
#
# Usage:
#   mkdir -p logs
#   sbatch train_dolci_instruct_plus_nemotron_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/dolci_instruct_plus_nemotron_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — same as Exp A (OLMo 3 paper Table 47)
RUN_NAME="${RUN_NAME:-dolci_instruct_plus_nemotron_sft}"
...
echo "=== Dolci Instruct + Nemotron FC SFT Training — Experiment F (Leonardo) ==="
