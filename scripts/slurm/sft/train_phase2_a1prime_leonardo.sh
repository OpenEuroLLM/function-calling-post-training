#!/usr/bin/env bash
#SBATCH --job-name=phase2-a1prime-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment A1' (step-matched A1): FC-only SFT on a step-matched
# subsample of (Dolci + GS), totaling 203,035 samples (= P1).
#
# A1' eliminates A1's step-count confound. Compare:
#   - A1' vs P1: GS-specific signal at fixed compute (volume-matched, step-matched,
#     recipe-identical; only composition varies).
#   - A1 vs A1': decomposes A1's forward delta into "GS signal" vs
#     "additional-step contribution."
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a1prime_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a1prime_leonardo.sh
...
echo "=== Phase 2 A1': Step-matched (Dolci+GS) FC SFT Training (Leonardo) ==="
