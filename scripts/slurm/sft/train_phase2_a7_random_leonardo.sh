#!/usr/bin/env bash
#SBATCH --job-name=phase2-a7-random-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A7_random: FC-only SFT with curated Dolci +
# nemotron_terminal random subsample (426,255 samples).
#
# Random-removal control for AMS filtering on nemotron_terminal. Random
# subsample of nemotron_terminal.jsonl to 223,220 (matching A7_ams volume)
# at seed=42. A7_ams vs A7_random is volume-matched, step-matched,
# recipe-identical; the comparison extends Claim 4's "AMS-targeted > random"
# test to nemotron_terminal (highest AMS rate among Wave 0 datasets, 40.77%).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a7_random_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a7_random_leonardo.sh
...
echo "=== Phase 2 A7_random: Curated Dolci + nemotron_terminal-random FC SFT Training ==="
