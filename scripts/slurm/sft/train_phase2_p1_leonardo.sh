#!/usr/bin/env bash
#SBATCH --job-name=phase2-p1-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment P1: FC-only SFT with curated Dolci FC (203,035 samples).
#
# Baseline: curated Dolci FC alone. All add-one experiments (A1/A2/A3) are
# compared against P1 to measure each dataset's forward contribution.
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_p1_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_p1_leonardo.sh
...
echo "=== Phase 2 P1: Curated Dolci FC SFT Training (Leonardo) ==="
