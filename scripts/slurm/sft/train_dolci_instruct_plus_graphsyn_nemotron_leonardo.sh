#!/usr/bin/env bash
#SBATCH --job-name=dolci-instruct-graphsyn-nemotron-sft
#SBATCH --nodes=8
...
# Experiment K: Full IT+FC SFT with Dolci-Instruct-SFT + GraphSyn FC + Filtered Nemotron FC on Leonardo.
#
# Hyperparameters identical to Experiment A/F (LR 8e-5, batch ~1M, 2 epochs, 8 nodes).
# Tests whether augmenting the full Dolci SFT mix with both GraphSyn FC and
# filtered Nemotron FC gives the best overall model.
# See fc_experiments_design.md and fc_experiments_log.md for rationale.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_dolci_instruct_plus_graphsyn_nemotron_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A)
#
# Usage:
#   mkdir -p logs
#   sbatch train_dolci_instruct_plus_graphsyn_nemotron_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/dolci_instruct_plus_graphsyn_nemotron_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — same as Exp A/F (OLMo 3 paper Table 47)
RUN_NAME="${RUN_NAME:-dolci_instruct_plus_graphsyn_nemotron_sft}"
...
echo "=== Dolci Instruct + GraphSyn + Filtered Nemotron FC SFT Training — Experiment K (Leonardo) ==="
