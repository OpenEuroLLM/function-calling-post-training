#!/usr/bin/env bash
#SBATCH --job-name=prepare-phase2-a3
#SBATCH --nodes=1
...
# Phase 2 Experiment A3: Tokenize curated Dolci FC + TxT360-AMS (365,200 samples).
#
...
#
# Prerequisites:
#   - Dolci-format JSONLs at $PHASE2_DIR/{dolci,txt360_ams}.jsonl
#     Generated by convert_openai_to_dolci_format.py from fcanalysis phase2_data.
#
# Usage:
#   mkdir -p logs
#   sbatch prepare_phase2_a3_data.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORK_DIR}/data/phase2_a3_tokenized}"
TOKENIZER="${TOKENIZER:-allenai/Olmo-3-7B-Instruct-SFT}"
DATASET_CACHE_DIR="${WORK_DIR}/local_dataset_cache"

echo "=== Phase 2 A3: Curated Dolci + TxT360-AMS FC Data Preparation ==="
...
for f in "$DOLCI_JSONL" "$TXT360_AMS_JSONL"; do
    if [ ! -f "$f" ]; then
...
python scripts/data/convert_sft_data_for_olmocore.py \
    ...
    --dataset_mixer_list "$DOLCI_JSONL" 1.0 "$TXT360_AMS_JSONL" 1.0 \
