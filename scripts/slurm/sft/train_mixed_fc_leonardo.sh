...
# Experiment E (Q1b): FC-only SFT with mixed Dolci FC + Nemotron FC data on Leonardo.
#
# Hyperparameters identical to Experiments B and C (LR 5e-5, batch 256K, 1 epoch).
# Tests whether mixing both FC datasets is better than either alone.
# See FC_EXPERIMENTS_LOG.md and fc_experiment_plan.md for rationale.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_mixed_fc_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A/B/C/D)
#
# Usage:
#   mkdir -p logs
#   sbatch train_mixed_fc_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/mixed_fc_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — same as Exp B/C
RUN_NAME="${RUN_NAME:-mixed_fc_sft}"
...
echo "=== Mixed FC SFT Training — Experiment E (Leonardo) ==="
