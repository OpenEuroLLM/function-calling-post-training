#!/usr/bin/env bash
#SBATCH --job-name=phase2-q2-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment Q2: FC-only SFT with curated Dolci + TxT360-random (365,200 samples).
#
...
# the A3 vs Q2 headline test. SEED selects DATASET_PATH=phase2_q2_tokenized_seed${SEED}
# (produced by SEED=${SEED} sbatch prepare_phase2_q2_data.sh), suffixes
# RUN_NAME/SAVE_FOLDER, and is also passed as --init_seed.
#
# Prerequisites:
#   1. Prepare tokenized data: SEED=${SEED} sbatch prepare_phase2_q2_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_q2_leonardo.sh                   # default (init_seed=33333)
#   SEED=42 sbatch train_phase2_q2_leonardo.sh            # replication k=42
#   SEED=7  sbatch train_phase2_q2_leonardo.sh            # replication k=7
#   SEED=13 sbatch train_phase2_q2_leonardo.sh            # replication k=13
...
# SEED=N → tokenized dir suffix _seedN, RUN_NAME/SAVE_FOLDER suffix _seedN, init_seed=N.
SEED="${SEED:-}"
SEED_SUFFIX="${SEED:+_seed${SEED}}"
INIT_SEED="${INIT_SEED:-${SEED:-33333}}"

DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_q2_tokenized${SEED_SUFFIX}}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — FC-only recipe
RUN_NAME="${RUN_NAME:-phase2_q2_fc_sft${SEED_SUFFIX}}"
...
echo "=== Phase 2 Q2: Curated Dolci + TxT360-random FC SFT Training (Leonardo) ==="
