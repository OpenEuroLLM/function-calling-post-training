...
# Experiment B (Q1): FC-only SFT with Dolci Tool-Use data on Leonardo.
#
# Hyperparameters identical to Experiment C (Nemotron FC) except max_epochs
...
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_dolci_fc_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A/D)
#
# Usage:
#   mkdir -p logs
#   sbatch train_dolci_fc_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/dolci_fc_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config
RUN_NAME="${RUN_NAME:-dolci_fc_sft}"
...
echo "=== Dolci FC SFT Training — Experiment B (Leonardo) ==="
