#!/usr/bin/env bash
#SBATCH --job-name=dolci-it-only-sft
#SBATCH --nodes=8
...
# Experiment D (Q3): IT-only SFT on Leonardo.
# Identical to Experiment A (train_dolci_instruct_leonardo.sh) except:
#   - Data: Dolci-Instruct-SFT minus Tool-Use (~1.9M IT-only samples)
#   - Run name: dolci_it_only_sft
#
# All hyperparameters match A exactly: LR 8e-5, batch ~1M tokens, 2 epochs, 32K seq.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_dolci_it_only_data.sh
#   2. Convert checkpoint:     sbatch convert_checkpoint_leonardo.sh (same as A)
#
# Usage:
#   mkdir -p logs
#   sbatch train_dolci_it_only_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/dolci_it_only_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — MUST match Experiment A exactly
RUN_NAME="${RUN_NAME:-dolci_it_only_sft}"
...
echo "=== Dolci IT-Only SFT Training — Experiment D (Leonardo) ==="
