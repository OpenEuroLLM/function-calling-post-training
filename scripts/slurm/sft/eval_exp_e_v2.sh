...
# Experiment E (v2): Mixed FC SFT (Dolci FC + Nemotron FC) — trimmed olmo3:adapt × 3 runs.
#
# Model: Think-SFT fine-tuned on Dolci FC + Nemotron FC combined (~544K samples, 1 epoch, step 4032).
# Checkpoint: mixed_fc_sft_hf (converted from OLMo-core).
#
...
#   mkdir -p logs
#   sbatch --cpus-per-task=32 eval_exp_e_v2.sh
...
MODEL_PATH="${WORK_DIR}/models/mixed_fc_sft_hf"
OUTPUT_BASE="${WORK_DIR}/eval-results/exp_e_mixed_fc_v2"
...
echo "=== Experiment E (v2): Mixed FC SFT — trimmed olmo3:adapt × 3 runs ==="
