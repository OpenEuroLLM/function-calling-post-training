#!/usr/bin/env bash
#SBATCH --job-name=phase2-q4-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Experiment Q4: FC-only SFT with curated Dolci + v1-dedup-random (385,927 samples).
#
# Random-removal control for the AMS filtering test on v1 (parallel to Q2 for TxT360).
# Compare A2' vs Q4: volume-matched, step-matched, recipe-identical; only the
...
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_q4_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_q4_leonardo.sh
...
echo "=== Phase 2 Q4: Curated Dolci + v1-dedup-random FC SFT Training (Leonardo) ==="
