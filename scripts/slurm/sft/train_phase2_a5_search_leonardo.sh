#!/usr/bin/env bash
#SBATCH --job-name=phase2-a5-search-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A5_search: FC-only SFT with curated Dolci + v2.search
# (208,998 samples).
#
# Tests v2.search's contribution to BFCL Web Search. v2.search is the only
# dataset in the mix with web_search supervision; only 1 filter variant
# possible (0 no-FC turns by construction → no AMS classification needed).
# FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a5_search_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a5_search_leonardo.sh
...
echo "=== Phase 2 A5_search: Curated Dolci + v2.search FC SFT Training (Leonardo) ==="
