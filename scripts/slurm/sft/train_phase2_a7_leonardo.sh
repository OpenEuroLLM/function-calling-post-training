#!/usr/bin/env bash
#SBATCH --job-name=phase2-a7-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 0 Experiment A7: FC-only SFT with curated Dolci + nemotron_terminal
# (438,800 samples).
#
# Tests nemotron_terminal's contribution to BFCL (shell command execution,
# iterative debugging, Memory via long shell output context). No AMS filter
# (no_AMS variant of the 3-variant terminal family; pair with A7_ams and
# A7_random). FC-only recipe: LR 5e-5, batch 256K, 1 epoch, 4 nodes.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_a7_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_a7_leonardo.sh
...
echo "=== Phase 2 A7: Curated Dolci + nemotron_terminal FC SFT Training (Leonardo) ==="
