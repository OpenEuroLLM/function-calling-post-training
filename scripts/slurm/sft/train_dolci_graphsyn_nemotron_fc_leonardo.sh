#!/usr/bin/env bash
#SBATCH --job-name=dolci-graphsyn-nemotron-fc-sft
#SBATCH --nodes=4
...
# Experiment I: FC-only SFT with Dolci FC + GraphSyn FC + Filtered Nemotron FC on Leonardo.
#
# Combines all three FC datasets. Hyperparameters identical to Experiments
# B, C, E, G, H (LR 5e-5, batch 256K, 1 epoch).
# Tests whether the 3-dataset FC mix is better than any 2-dataset combination.
# See fc_experiments_design.md and fc_experiments_log.md for rationale.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_dolci_graphsyn_nemotron_fc_data.sh
#   2. OLMo-core checkpoint must be available (same as Exp A/B/C/D/E/F/G/H)
#
# Usage:
#   mkdir -p logs
#   sbatch train_dolci_graphsyn_nemotron_fc_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/dolci_graphsyn_nemotron_fc_tokenized}"
BASE_CKPT="${BASE_CKPT:-${WORK_DIR}/models/OLMo-3-7B-Think-SFT-olmocore/model_and_optim}"

# Training config — same as Exp B/C/E/G/H
RUN_NAME="${RUN_NAME:-dolci_graphsyn_nemotron_fc_sft}"
...
echo "=== Dolci + GraphSyn + Filtered Nemotron FC SFT Training — Experiment I (Leonardo) ==="
