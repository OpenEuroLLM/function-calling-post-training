#!/usr/bin/env bash
#SBATCH --job-name=phase2-centerpiece-turndrop-v2-fc-sft
#SBATCH --nodes=4
...
# Phase 2 Wave 1 v2 (clean 2-stage labels) centerpiece_turndrop_v2: FC-only SFT
# on the 5-dataset centerpiece with unjustified turn-range drop (4 categories,
# clean labels) applied to v1/v2.IA/TxT360. 891,441 samples. dolci + graphsyn
# passthrough. Same FC-only recipe as the v1 centerpieces: LR 5e-5, batch 256K,
# 1 epoch, 4 nodes / 16 GPUs. v2 counterpart of centerpiece_turndrop.
#
# Prerequisites:
#   1. Prepare tokenized data: sbatch prepare_phase2_centerpiece_turndrop_v2_data.sh
#   2. OLMo-core checkpoint must be available
#
# Usage:
#   mkdir -p logs
#   sbatch train_phase2_centerpiece_turndrop_v2_leonardo.sh
...
DATASET_PATH="${DATASET_PATH:-${WORK_DIR}/data/phase2_centerpiece_turndrop_v2_tokenized}"
...
RUN_NAME="${RUN_NAME:-phase2_centerpiece_turndrop_v2_fc_sft}"
...
echo "=== Phase 2 centerpiece_turndrop_v2: clean-label turn-drop FC SFT Training (Leonardo) ==="
